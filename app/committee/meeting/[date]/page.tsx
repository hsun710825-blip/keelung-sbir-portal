...
export default async function CommitteeMeetingPage({ params }: PageProps) {
  ...
  const evaluations = await prisma.evaluation.findMany({
    ...
    select: { applicationId: true, score: true, status: true },
  });
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <header className="mb-6 flex flex-col gap-4 rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              href={`/committee/meeting/${meetingDate}/summary`}
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-5 py-4">
            ...
            <p className="text-sm text-slate-500">共 {rows.length} 案（議程表 {config.cases.length} 案）</p>
          </div>
