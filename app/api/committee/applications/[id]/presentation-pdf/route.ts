...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const filename = `${(app.title || "presentation").replace(/[^\w\u4e00-\u9fff.-]+/g, "_")}_簡報.pdf`;
    ...
    console.error("[committee/presentation-pdf] download failed:", error);
    return NextResponse.json({ ok: false, error: "無法讀取簡報 PDF" }, { status: 502 });
  }
}
