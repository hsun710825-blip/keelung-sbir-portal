...
import { computeSettlementSavePayload, roundFunding } from "@/lib/settlementFormulas";
...
export async function saveSettlementRowAction(
  ...
  const parseOptionalFloat = (key: string) => {
    const t = String(formData.get(key) || "").trim();
    if (!t) return null;
  ...
  const payload = computeSettlementSavePayload({
    appliedSubsidy: roundFunding(parseOptionalInt("appliedSubsidy")),
    appliedSelfFund: roundFunding(parseOptionalInt("appliedSelfFund")),
    appliedTotal: roundFunding(parseOptionalInt("appliedTotal")),
    tierRate: parseOptionalFloat("tierRate"),
  });
